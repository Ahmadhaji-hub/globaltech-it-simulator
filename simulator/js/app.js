...
// ---- MOBILE MENU ---- (محدّث مع swipe gesture + iOS fixes)
function initMobileMenu() {
  ...
  function openSidebar() {
    ...
    document.body.style.overflow = 'hidden'; /* منع scroll الخلفية على iOS */
  }
  ...
  /* FIX: إغلاق السايدبار عند النقر على أي nav link على موبايل */
  document.querySelectorAll('.sidebar-nav a').forEach(link => {
  ...
  /* FIX: Swipe gesture لفتح/إغلاق السايدبار */
  let touchStartX = 0;
  let touchStartY = 0;
  ...
  document.addEventListener('touchend', (e) => {
    ...
    if (dx > SWIPE_THRESHOLD && touchStartX < EDGE_ZONE) {
      openSidebar(); /* swipe right من الحافة */
    } else if (dx < -SWIPE_THRESHOLD && sidebar.classList.contains('open')) {
      closeSidebar(); /* swipe left يُغلق */
    }
  }, { passive: true });
}
...
function initIOSViewportFix() {
  /* FIX: iOS Safari 100vh bug — نحسب الـ visual viewport الفعلي */
  function setVH() {
